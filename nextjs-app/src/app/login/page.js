...
export default function LoginPage() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      // Login with email/phone + password ONLY — role is auto-detected from DB
      const res = loginWithCredentials(identifier, password);
      if (res.success) {
        setSuccess(true);
        setTimeout(() => {
          if (res.role === 'Admin') router.push('/admin');
          else if (res.role === 'Supervisor') router.push('/supervisor');
      ...
      <AnimatePresence>
        {success ? (
          ...
          >
            ...
            <motion.div className="login-success-text" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
              Welcome back!
            </motion.div>
            <motion.div className="login-success-sub" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }}>
          ...
          >
            <motion.div className="login-logo" whileHover={{ scale: 1.05, rotate: 5 }} whileTap={{ scale: 0.95 }}>
              <Shield size={36} />
            </motion.div>
            ...
            <motion.p className="login-subtitle" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
