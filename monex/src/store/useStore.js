...
const useStore = create(
  ...
  addDocumentToUser: async (userId, document) => {
    ...
    // 1. Build updated documents list immediately
    const updatedDocuments = [...(user.documents || []), { ...document }];

    // 2. Update local Zustand state instantly (no blocking await)
    set((state) => ({
      users: state.users.map(u =>
        u.id === userId ? { ...u, documents: updatedDocuments } : u
      ),
      // Also update currentUser so the Settings page re-renders immediately
      currentUser: state.currentUser?.id === userId
        ? { ...state.currentUser, documents: updatedDocuments }
        : state.currentUser,
    }));

    // 3. Persist to Firestore in the background (non-blocking, store as base64)
    setDoc(doc(db, 'users', userId), { documents: updatedDocuments }, { merge: true })
